...
computePersonalAsync = (playerName, done)->
  try
    profile = {
      "name": "",
      "rating": 0,
    ...
    playerGames = await db.PlayerGame.findAll({where: {PlayerId: player.id}})
    games = (await pg.getGame() for pg in playerGames)
...
module.exports = (playerName)->
  new Promise( (resolve, reject)->
    computePersonalAsync(playerName, (err, profile)->
      reject(err) if err
      resolve(profile) if not err
    )
  )